...
시그널과 리스크 게이트는 `signal-engine.py`로 **정량적으로** 판단합니다.
Claude의 자연어 판단이 아닌 코드화된 로직으로 일관성을 보장합니다.

엔진 경로: `/Users/aiden-kwak/Desktop/Personal/Stock/toss-trading-system/scripts/signal-engine.py`

### 1. 손절/익절 체크 (check-positions)

보유종목을 순회하며 자동 시그널 생성:

```bash
python3 signal-engine.py check-positions --positions '<tossctl positions json>' --config '{}'
```

| 조건 | 액션 | 긴급도 |
|------|------|--------|
| 수익률 ≤ -3% (손절선) | `SELL_STOP_LOSS` | HIGH |
| 수익률 ≥ +7% (익절선) | `SELL_TAKE_PROFIT` | MEDIUM |
| 일일 등락 ≤ -5% (급락) | `ALERT_SHARP_DROP` | HIGH |
| 그 외 | `HOLD` | NONE |

### 2. 매수 시그널 평가 (evaluate-buy)

종목을 0-100점으로 채점하여 등급 산출:

```bash
python3 signal-engine.py evaluate-buy --quote '<json>' --portfolio '<json>' --config '{}'
```

| 항목 | 배점 | 기준 |
|------|------|------|
| 거래량 | 0-30 | 5M→10, 10M→20, 20M→25, 50M+→30 |
| 모멘텀 | 0-25 | +1~5%→25, +5~10%→15, +10%↑→5(과열) |
| 가격위치 | 0-25 | 전일比 -2~5%→25(저가매수), ±2%→20, +5%↑→5 |
| 포트폴리오 적합 | 0-20 | 여력 충분→20, 부족→10, 없음→0 |
...
**A/B 등급만 매수 진행합니다. C 이하는 관망.**

### 3. 리스크 게이트 (risk-gate)

모든 매수 전 반드시 통과해야 하는 3개 체크:

```bash
python3 signal-engine.py risk-gate --portfolio '<json>' --today-pnl -35000 --active-positions 2
```

| 체크 | 기준 | 미통과 시 |
|------|------|-----------|
| 일일 손실 한도 | 당일 손실 > 총자산의 -2% | 당일 거래 전면 중단 |
| 동시 포지션 한도 | 현재 포지션 ≥ 2개 | 신규 매수 차단 |
| 투자 여력 | 주문가능금액 < 최소 필요액 | 신규 매수 차단 |

**3개 모두 통과해야 매수 가능. 하나라도 실패하면 차단.**

### 매 사이클 판단 흐름

```
1. check-positions → SELL_STOP_LOSS/TAKE_PROFIT 있으면 즉시 매도
2. risk-gate → 미통과면 매수 스킵
3. evaluate-buy → A/B 등급 종목만 매수 대상
4. 보호종목 체크 → hook이 시스템 레벨에서 차단
```
